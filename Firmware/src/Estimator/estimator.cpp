...
{
    // update board orientation this is applied when converthing back to sensor frame where the
    // orientaiton of sensor matters upside down should be retireved from config file
    ...
    if (dt > update_frequency)
    {
        ...
        if (_systemstatus.flagSetOr(SYSTEM_FLAG::ERROR_IMU))
        {
            if (_systemstatus.flagSetOr(SYSTEM_FLAG::ERROR_GPS) &&
                _systemstatus.flagSetOr(SYSTEM_FLAG::ERROR_BARO))
            {
                changeEstimatorState(ESTIMATOR_STATE::NOSOLUTION,
                                     "no data, cannot compute navigation solution");
                return;
            }
            ...
            {
                // gps only update, no fusion as filtering this data will only result in a worse
                // solution
        ...
        {  // we have imu so calculate orientation and update localizationkf
            ...
            {
                computeOrientation(raw_sensors.accelgyro.gx, raw_sensors.accelgyro.gy,
                                   raw_sensors.accelgyro.gz, raw_sensors.accelgyro.ax,
                                   raw_sensors.accelgyro.ay, raw_sensors.accelgyro.az, dt_seconds);

                if (_homeSet)
                {
                    changeEstimatorState(ESTIMATOR_STATE::PARTIAL_NO_MAG,
                                         "no mag, heading unreliable");
                }
            }
            else
            {
                computeOrientation(
                    raw_sensors.accelgyro.gx, raw_sensors.accelgyro.gy, raw_sensors.accelgyro.gz,
                    raw_sensors.accelgyro.ax, raw_sensors.accelgyro.ay, raw_sensors.accelgyro.az,
                    raw_sensors.mag.mx, raw_sensors.mag.my, raw_sensors.mag.mz, dt_seconds);
            }
            // transform angular rates from body frame to earth frame
            updateAngularRates(raw_sensors.accelgyro.gx, raw_sensors.accelgyro.gy,
                               raw_sensors.accelgyro.gz);
            localizationkf.accelUpdate(getLinearAcceleration(raw_sensors.accelgyro.ax,
            ...
            {
                // localizationkf.HaccelUpdate(getLinearAcceleration(raw_sensors.accel.ax,
                // raw_sensors.accel.ay, raw_sensors.accel.az) * g);
            }
        }

        if (!_homeSet)
        {
            changeEstimatorState(ESTIMATOR_STATE::NO_HOME,
                                 "Home not set, Localization not avaliable");
            return;
        }
        ...
        if (_systemstatus.flagSetOr(SYSTEM_FLAG::ERROR_GPS))
        {
            if (_systemstatus.flagSetOr(SYSTEM_FLAG::ERROR_BARO))
            {
                ...
                changeEstimatorState(
                    ESTIMATOR_STATE::PARTIAL_NO_GPS_NO_BARO,
                    "no gps and baro, position and velocity estimates unreliable!");
                predictLocalizationKF(dt_seconds);
                return;
            }
            // baro only update
            changeEstimatorState(ESTIMATOR_STATE::PARTIAL_NO_GPS,
                                 "no gps, N and E positon and velocity estimates unreliable!");
        }
        else
        {
        ...
        if (!_systemstatus.flagSetOr(SYSTEM_FLAG::ERROR_IMU, SYSTEM_FLAG::ERROR_GPS,
                                     SYSTEM_FLAG::ERROR_BARO, SYSTEM_FLAG::ERROR_MAG,
                                     SYSTEM_FLAG::ERROR_HACCEL))
        {
...
void Estimator::setHome(const SensorStructs::raw_measurements_t &raw_sensors)
{
    ...
    RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
        ...
        std::to_string(state.gps_launch_long) + " Alt: " + std::to_string(state.gps_launch_alt) +
        " Baro Ref Alt: " + std::to_string(state.baro_ref_alt));
    // update reference coordinates in position estimation
    localizationkf.updateGPSReference(state.gps_launch_lat, state.gps_launch_long,
                                      state.gps_launch_alt);
...
void Estimator::updateOrientation()
{
    ...
    // state.eulerAngles = quat2rpy(state.orientation);
    // state.rocketOrientation = (refOrientation * state.orientation *
    // refOrientation.inverse()).normalized(); state.rocketEulerAngles =
    // quat2rpy(state.rocketOrientation);
    ...
    // // state.rocketEulerAngles =  state.rocketOrientation.toRotationMatrix().eulerAngles(0, 1,
    // 2); state.rocketEulerAngles = Eigen::Vector3f(roll,pitch,yaw);
...
void Estimator::changeBeta(float beta) { madgwick.setBeta(beta); }
...
void Estimator::setIgnitionTime(uint32_t time)
{
    ...
    RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("Ignition commanded at " +
                                                            std::to_string(time));
}
...
void Estimator::setLiftoffTime(uint32_t time)
{
    ...
    RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("Liftoff detected at " +
                                                            std::to_string(time));
}
...
void Estimator::setApogeeTime(uint32_t time)
{
    ...
    RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("Apogee detected at " +
                                                            std::to_string(time));
}
...
void Estimator::setRefOrientation(JsonObjectConst conf)
{
    ...
    try
    {
        ...
        {
            RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
                "No Orientation Type Given, using Default!");
            return;
        }
        ...
        {
            refOrientation = Eigen::Quaternionf(
                {getIfContains<float>(conf, "w"), getIfContains<float>(conf, "x"),
                 getIfContains<float>(conf, "y"), getIfContains<float>(conf, "z")});
            refOrientation.normalize();  // ensure ref orinetation is normalized
        }
        ...
        {
            RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
                "Illegal Orientation Type Given, using Default!");
            return;
        }
    }
    ...
    {
        RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
            "Exception occured while processing orientation! - " + std::string(e.what()));
