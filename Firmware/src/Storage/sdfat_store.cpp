...
SdFat_Store::SdFat_Store(SPIClass &spi, const uint8_t cs, const uint32_t frequency,
                         RicCoreThread::Lock_t &spiBusLock, bool dedicatedSPI,
...
SdFat_Store::SdFat_Store(SPIClass &spi, const uint8_t cs, const uint32_t frequency,
                         bool dedicatedSPI, Types::CoreTypes::SystemStatus_t *systemstatus)
...
std::unique_ptr<WrappedFile> SdFat_Store::_open(std::string_view path, store_fd fileDesc,
                                                FILE_MODE mode, size_t maxQueueSize)
{
    return std::make_unique<SdFat_WrappedFile>(
        filesys.open(std::string(path).c_str(), static_cast<oflag_t>(mode)), fileDesc, *this, mode,
        maxQueueSize);
}
...
bool SdFat_Store::_ls(std::string_view path, std::vector<directory_element_t> &directory_structure)
{
    ...
    {                               // while child file is valid
        ...
        entry.name =
            std::string{filename};  // convert char array to string and pass into entry structure
...
bool SdFat_Store::_mkdir(std::string_view path) { return filesys.mkdir(std::string(path).c_str()); }
