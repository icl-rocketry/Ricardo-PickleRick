...
SimpleEngine::SimpleEngine(uint8_t id, JsonObjectConst engineConfig,
                           const Types::LocalPyroMap_t &localPyroMap,
    ...
    : Engine(id, networkmanager, handlerServiceID),
      _state({
          static_cast<uint8_t>(ENGINE_RUN_STATE::SHUTDOWN),
      })
{
    ...
    if (igniterType == "local_pyro")
    {
        ...
        _igniter =
            std::make_unique<RemoteActuatorAdapter<Types::LocalPyro_t>>(id, localPyro, getLogCB());
    }
    else if (igniterType == "net_actuator")
    {
        ...
        _igniter = std::make_unique<NetworkActuator>(0, igniterAddress, _handlerServiceID,
                                                     igniterDestinationService, networkmanager,
                                                     getLogCB());
        addNetworkCallbackFunction(
            igniterAddress, igniterDestinationService,
            [this](packetptr_t packetptr) {
                dynamic_cast<NetworkActuator *>(_igniter.get())
...
void SimpleEngine::updateState() { _igniter->updateState(); }

uint8_t SimpleEngine::flightCheck()
{
    uint8_t flightcheck_result =
        _igniter->flightCheck(_networkRetryInterval, _componentStateExpiry, "SimpleEngine");
...
void SimpleEngine::ignite()
{
    ...
    _state.runState = static_cast<uint8_t>(
        ENGINE_RUN_STATE::RUNNING);  // goes straight to running state as there is no igntion time
}

void SimpleEngine::shutdown()
{  // the engine cant be shut down lol
    Engine::shutdown();
    _state.runState = static_cast<uint8_t>(ENGINE_RUN_STATE::SHUTDOWN);
}

void SimpleEngine::armEngine() { _igniter->arm(); }

void SimpleEngine::disarmEngine() { _igniter->disarm(); }
