...
ADC_VRailMonitor::ADC_VRailMonitor(std::string_view vrail_name, const uint8_t pin, const float r1,
                                   const float r2)
    : _name(vrail_name),
      _pin(pin),
...
void ADC_VRailMonitor::setup(int maxVoltage, int lowVoltage, int minVoltage)
{
    ...
    if (error)
    {
        RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
            "Vrail Monitor Failed to initialize!");
        return;
    }
...
void ADC_VRailMonitor::update(SensorStructs::ADC_V_RAIL_t &data)
{
    ...
    if (millis() - prevSampleTime >= sampleDelta)
    {
        ...
        {
            RicCoreLogging::log<LOG_TARGET>(_name + ": low voltage, at " +
                                            std::to_string(data.volt) + "mV");
        ...
        {
            data.percent = int(
                ((float)(data.volt - _minVoltage) / (float)(_maxVoltage - _minVoltage)) * 100.0);
        }
    }
}
