...
void PCA9685::writeWidth(uint8_t pin, uint32_t usec) { writeDuty(pin, toCounts(usec)); }
...
uint32_t PCA9685::toCounts(uint32_t usec)
{
    ...
    float fractionOfPeriod =
        (static_cast<float>(usec) * static_cast<float>(m_freq)) / static_cast<float>(1e6);
...
void PCA9685::setFrequency(uint32_t freq)
{
    ...
    float prescale_temp = std::round((static_cast<float>(OSC_FREQUENCY) /
                                      (static_cast<float>(4096) * static_cast<float>(freq))) -
                                     1);
    prescale_temp = std::min(std::max(prescale_temp, 0.f),
                             255.f);  // ensure float is properly bounded for uint8_t
    // bound prescale to hardware limits
    uint8_t prescale = static_cast<uint8_t>(
        std::max(std::min(static_cast<uint16_t>(prescale_temp), static_cast<uint16_t>(0xff)),
