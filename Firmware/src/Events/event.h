...
class Event{
    public:
        Event(int eventID,condition_t condition,action_t action,bool singleUse,uint16_t actionCooldown):
        _eventID(eventID),
        ...
        {
             RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("Event " + std::to_string(_eventID) + " created");
        };
