...
flightVariable_t FlightVariables::TimeSinceApogee(int arg) { return timeSince(_state.apogeeTime); }
...
flightVariable_t FlightVariables::Position(int arg) { return getComponent(_state.position, arg); }

flightVariable_t FlightVariables::Velocity(int arg) { return getComponent(_state.velocity, arg); }
...
flightVariableFunc_t FlightVariables::get(const std::string& funcName)
{
    ...
    return [this, funcName, funcptr](int idx)
    {
        ...
        std::cout << "[FlightVar] :" + variablename +
                         " called with value : " + std::to_string(var.value_or(0))
                  << std::endl;
#endif
        return var;
    };
