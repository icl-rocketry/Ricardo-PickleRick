...
SdFat_WrappedFile::SdFat_WrappedFile(FsFile file, store_fd fileDesc, SdFat_Store& store,
                                     FILE_MODE mode, size_t maxQueueSize)
    : WrappedFile(store, fileDesc, mode, maxQueueSize), _file(file)
{
}

RicCoreThread::Lock_t& SdFat_WrappedFile::getDevLock() { return store.get_lock(); };
...
void SdFat_WrappedFile::_read(std::vector<uint8_t>& dest)
{
    ...
    if (numBytes == -1)
    {
        // could be io error or read called when *underlying* file not opened, idk if we can
        // differentiate this
