...
void EventHandler::setup(JsonArrayConst event_config)
{
    ...
    for (JsonObjectConst jsonEvent : event_config)
    {
        ...
        std::string eventName =
            getIfContains<std::string>(jsonEvent, "name", std::string("Unnamed"));
        ...
        uint16_t actionCooldown =
            getIfContains<uint16_t>(jsonEvent, "cooldown", 1000);  // default 1 second cooldown

        //! Null configs for the condition config and the action config are handleld in their
        ...
        {                           // either it doesnt exist or "action":null
        ...
        _eventList.at(eventID) = std::make_unique<Event>(eventID, eventName, eventCondition,
                                                         eventAction, fire_mode, actionCooldown);
...
action_t EventHandler::configureAction(JsonVariantConst actions)
{
    if (actions.is<JsonObjectConst>())
    {
        ...
        if (actionType == "engine")
        {
            ...
            return [actionParam, actionID, this]()
            { _enginehandler.getActionFunc(actionID)(actionParam); };
        }
        else if (actionType == "deployment")
        {
            ...
            int actionParam = actionJson["param"];
            return [actionParam, actionID, this]()
            { _deploymenthandler.getActionFunc(actionID)(actionParam); };
        }
        ...
        else
        {
            ...
            RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
                "Bad Action Type, continuing with null action");
            return []() {};
        }
    }
    ...
    else
    {
        ...
        RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>(
            "Bad Action config, continuing with null action");
        return []() {};  // null action
    }
}
...
condition_t EventHandler::configureCondition(JsonVariantConst condition, uint8_t recursion_level)
{
    if (recursion_level > condition_recursion_max_depth)
    {
        throw std::runtime_error(
            "EventHandler max recursion depth reached!");  // this needs to fail quietly too
    }

    if (!condition.is<JsonObjectConst>())
    {
        throw std::runtime_error(
            "EventHandler Invalid type deserialized : " + condition.as<std::string>() +
            " depth : " + std::to_string(recursion_level));
    }
    ...
    {
        auto subconditionarray =
            conditionJson["condition"]
                .as<JsonArrayConst>();  // required explict cast to JsonArray type
        size_t arraysize = subconditionarray.size();

        if (arraysize == 0)
        {
            throw std::runtime_error(
                "EventHandler no conditions provided");  // we can fail safe here by returning false
        }
        if (arraysize == 1)
        {
            return configureCondition(subconditionarray[0],
                                      recursion_level);  // no change to recursion depth
        }
...
// get initial condition
        ...
        for (int i = 1; i < arraysize; i++)
        {
            ...
            conditionCombination = ConditionCombination(
                conditionCombination, configureCondition(subconditionarray[i], recursion_level + 1),
                op);
#ifdef _RICDEBUG
            _decisiontree += " ) ";
#endif
...
condition_t EventHandler::configureFlightVarCondition(JsonObjectConst conf)
{
    ...
    // if it is the only condition and u are testing time_triggered then the event will never
    // trigger if the conditon is 'and' with another condition it will again never trigger if the
    // conditon is or'ed then if the other condition is met, the event will trigger. If the conditon
    // comparing time trigered is less than, the result is no different to just testing the other
    // conditon. If using more than, we effectivley latch the output of the event, it will always be
    // on meaning the action will be continually called
    ...
    return Condition(
        _flightvariables.get(conf["flightVar"].as<std::string>()), conf["component"].as<int>(),
        conf["threshold"].as<float>(),
        op);  // while this looks like we are returning an Condtion object, as the return
              // type of the function is std::function<bool()> and the operator() is
              // defined for condtion, we are in fact returning a callable function
}
...
condition_t EventHandler::configureLocalPyroCondition(JsonObjectConst conf)
{
    ...
    {
        auto localPyro = RemoteActuatorAdapter<Types::LocalPyro_t>(
            0, *m_localPyroMap.at(localPyroChannel), [](std::string msg) {});
