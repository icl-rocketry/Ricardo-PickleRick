...
void ICM_20608::readGyro(float &x, float &y, float &z)
{
    ...
    std::array<float, 3> gyro = axeshelper(std::array<float, 3>{
        (float)(xi + offset_gx) * gyro_lsb_to_degs, (float)(yi + offset_gy) * gyro_lsb_to_degs,
...
void ICM_20608::readAccel(float &x, float &y, float &z)
{
    ...
    std::array<float, 3> accel = axeshelper(std::array<float, 3>{
        (float)(xi + offset_ax) * accel_lsb_to_g, (float)(yi + offset_ay) * accel_lsb_to_g,
...
void ICM_20608::writeAccelGyroBias()
{
    ...
    if (!pref.putShort("gxBias", offset_gx))
    {
        RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("nvs error while writing");
    };
    if (!pref.putShort("gyBias", offset_gy))
    {
        RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("nvs error while writing");
    };
    if (!pref.putShort("gzBias", offset_gz))
    {
        RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("nvs error while writing");
    };
