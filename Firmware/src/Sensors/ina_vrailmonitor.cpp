...
INA_VRailMonitor::INA_VRailMonitor(std::string_view vrail_name, TwoWire& wire, uint8_t address)
    : _name(vrail_name), m_ina(address, wire), _maxVoltage(0), _lowVoltage(0), _minVoltage(0){};
...
void INA_VRailMonitor::update(SensorStructs::INA_V_RAIL_t& data)
{
    ...
    {
        RicCoreLogging::log<LOG_TARGET>(_name + ": low voltage, at " + std::to_string(data.volt) +
                                        "mV");
    ...
    {
        data.percent =
            int(((float)(data.volt - _minVoltage) / (float)(_maxVoltage - _minVoltage)) * 100.0);
    }
}
