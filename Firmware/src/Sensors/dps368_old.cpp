...
//     : Dps3xx(), _spi(spi), _systemstatus(systemstatus), _cs(cs), _initialized(false)
// {
// }
...
//     _spi.beginTransaction(SPISettings(DPS3xx__SPI_MAX_FREQ, MSBFIRST, SPI_MODE3));
//     _spi.transfer(0xFF);  // start at PSR_B2
//     _spi.endTransaction();

//     begin(_spi, _cs);
//     // the dps library, while it pretends to check the product id and rev id, it actually only
//     // checks if it gets a response, so here we will proerply check if the prod id and rev id are as
//     // expected......

//     if (m_productID != 0 || m_revisionID != 1)
//     {
//         _systemstatus.newFlag(SYSTEM_FLAG::ERROR_BARO,
//                               "DPS368 failed to respond with expected prod and rev ID, Prod Id:" +
//                                   std::to_string(m_productID) +
//                                   ", Rev Id: " + std::to_string(m_revisionID));
//         return;
//     }
...
//     {
//         _systemstatus.newFlag(SYSTEM_FLAG::ERROR_BARO,
//                               "DPS368 failed to start with code: " + std::to_string(error));
//         return;
//     }
//     // start measure both cont enables FiFo, however we want to disable FiFo so that the result
//     // register is always the most up to date reading and we dont particualrly care about historical
//     // readings.
...
//     // for some reason the dps requires a 'dummy' transaction. Probbaly due to the change of clock
//     // polarity messing clocking the data out or somethin like that not really worth trying to fix
//     // any more than this. But if u are bored go for it. Maybe I should add a wasted hours counter
//     // on this
//     _spi.beginTransaction(SPISettings(DPS3xx__SPI_MAX_FREQ, MSBFIRST, SPI_MODE3));
...
//             "nvs failed to start. Can't write DPS calibration values");
//         return;
//     }

//     if (!pref.putFloat("TempRef", refTemp))
//     {
//         RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("nvs error while writing");
//     }
//     if (!pref.putFloat("PressRef", refPress))
//     {
//         RicCoreLogging::log<RicCoreLoggingConfig::LOGGERS::SYS>("nvs error while writing");
//     }
// }
