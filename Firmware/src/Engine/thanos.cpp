#include "Engine/thanos.h"

Thanos::Thanos(uint8_t id, JsonObjectConst engineConfig,
    ...
    : Engine(id, networkmanager, handlerServiceID)
{
    ...
    _engine = std::make_unique<NetworkActuator>(
    ...
    _igniter = std::make_unique<NetworkActuator>(
        1, getIfContains<uint8_t>(igniterConf, "address"), handlerServiceID,
    ...
    _oxVentValve = std::make_unique<NetworkActuator>(
        2, getIfContains<uint8_t>(oxVentValveConf, "address"), handlerServiceID,
        getIfContains<uint8_t>(oxVentValveConf, "destination_service"), _networkmanager,
        getLogCB());
    ...
    _fuelVentValve = std::make_unique<NetworkActuator>(
        3, getIfContains<uint8_t>(fuelVentValveConf, "address"), handlerServiceID,
        getIfContains<uint8_t>(fuelVentValveConf, "destination_service"), _networkmanager,
        getLogCB());
    ...
    addComponentNetworkCallback(_fuelVentValve.get(), fuelVentValveConf,
                                addNetworkCallbackFunction);

    auto fuelPrssValveConf = getIfContains<JsonObjectConst>(engineConfig, "fuelPrssValve");
    _fuelPrssValve = std::make_unique<NetworkActuator>(
        4, getIfContains<uint8_t>(fuelPrssValveConf, "address"), handlerServiceID,
        getIfContains<uint8_t>(fuelPrssValveConf, "destination_service"), _networkmanager,
        getLogCB());
    ...
    addComponentNetworkCallback(_fuelPrssValve.get(), fuelPrssValveConf,
                                addNetworkCallbackFunction);
...
uint8_t Thanos::flightCheck()
{
    ...
    res += _engine->flightCheck(_networkRetryInterval, _componentStateExpiry,
                                "Engine:" + std::to_string(getID()));
    res += _igniter->flightCheck(_networkRetryInterval, _componentStateExpiry,
                                 "Engine:" + std::to_string(getID()));
    res += _oxVentValve->flightCheck(_networkRetryInterval, _componentStateExpiry,
                                     "Engine:" + std::to_string(getID()));
    res += _fuelVentValve->flightCheck(_networkRetryInterval, _componentStateExpiry,
                                       "Engine:" + std::to_string(getID()));
    res += _fuelPrssValve->flightCheck(_networkRetryInterval, _componentStateExpiry,
                                       "Engine:" + std::to_string(getID()));
