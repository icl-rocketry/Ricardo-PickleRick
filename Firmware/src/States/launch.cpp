...
Launch::Launch(System &system)
    : State(SYSTEM_FLAG::STATE_LAUNCH, system.systemstatus), _system(system){};
...
void Launch::initialize()
{
    ...
    _system.commandhandler.enableCommands(
        {Commands::ID::Ignition, Commands::ID::Launch_Abort, Commands::ID::Liftoff_Override});
...
Types::CoreTypes::State_ptr_t Launch::update()
{
    ...
    // if (!_system.systemstatus.flagSet(SYSTEM_FLAG::ERROR_FLIGHTCHECK) &&
    // _system.estimator.getData().acceleration(2) < -1){ // launch acceleration threshold
    // comparison of down acceleration with a threshold of 1.5 g idk if this is okay lol?
    if (_system.estimator.getData().acceleration(2) < -(2 * 9.81) &&
