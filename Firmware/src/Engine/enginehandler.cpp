...
void EngineHandler::setupIndividual_impl(size_t id, JsonObjectConst engineconfig)
{
    ...
    {
        addObject(std::make_unique<SimpleEngine>(id, engineconfig, m_localPyroMap,
                                                 getaddNetworkCallbackFunction(id), _networkmanager,
                                                 _serviceID));
    }
    else if (type == "Hypnos")
    {
        addObject(std::make_unique<Hypnos>(id, engineconfig, getaddNetworkCallbackFunction(id),
                                           _networkmanager, _serviceID));
    }
    else if (type == "Thanos")
    {
        addObject(std::make_unique<Thanos>(id, engineconfig, getaddNetworkCallbackFunction(id),
                                           _networkmanager, _serviceID));
    }
    else if (type == "ThanosR")
    {
        addObject(std::make_unique<ThanosR>(id, engineconfig, getaddNetworkCallbackFunction(id),
                                            _networkmanager, _serviceID));
    }
    else
    {
...
{
    return [this, engineID](uint8_t source, uint8_t source_service,
                            std::function<void(std::unique_ptr<RnpPacketSerialized>)> callback,
                            bool throwOnError)
    {
        auto wrappedCallback =
            [this, callback, engineID](std::unique_ptr<RnpPacketSerialized> packet_ptr)
        {
