...
#include "Sound/Melodies/pitches.h"
...
{
public:
    melody_t(std::array<melody_note_t, S> array_in, bool priority) : note_sequence(array_in),
                                                                     _priority(priority){};

    melody_note_t getNote(int i) override { return note_sequence.at(i); }; // bounds checking
    bool getPriority() override { return _priority; };
